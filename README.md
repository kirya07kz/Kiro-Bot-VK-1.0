...
* **Автоответчик на базе SQLite:** Все вопросы и ответы хранятся в локальной базе данных. Поддержка текстовых ответов и вложений (фото, видео, документы).
...
# Установка APK (Debug или Release)
install_apk.bat
```
...
- [ ] Поддержка команд (например, `/help`, `/stats`)
- [ ] Расписание работы бота (включение/выключение по времени)
